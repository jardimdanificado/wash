...
const DEFAULT_SYSROOT = {
  ...
  "sysroot/include/stddef.h": "",
...
async function initCompiler() {
    ...
    if (!ccWasmBytes) {
        const wasmRes = await fetch("cc.wasm");
        if (!wasmRes.ok) {
            throw new Error(`cc.wasm not found (${wasmRes.status}). Ensure cc.wasm is placed in the studio directory or deploy via GitHub Pages.`);
        }
...
self.onmessage = async (e) => {
    const { id, type, filename, code, entryPoint } = e.data;
    ...
    try {
        ...
        const srcFileName = filename.endsWith(".c") ? filename : (filename + ".c");
        const outFileName = srcFileName.replace(/\.c$/, ".wasm");

        rootContents.set(srcFileName, new File(new TextEncoder().encode(code)));
